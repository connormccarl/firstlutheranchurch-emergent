/**
 * Optional admin seeding helper. Idempotent — safe to call on every cold start.
...
export async function seedAdmin(opts: SeedOptions = {}): Promise<void> {
  ...
  // Bump role/active flag if needed
  await query(
    `UPDATE users SET role = 'admin', is_active = true WHERE id = $1`,
    [existing.id],
  );

  // If the env password differs from the stored hash, rotate it.
