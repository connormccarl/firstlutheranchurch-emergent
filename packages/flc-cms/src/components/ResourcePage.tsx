...
import type { ResourceDef, ResourceRecord } from "../types.js";
import { DataTable } from "./DataTable.js";
import { RecordForm } from "./RecordForm.js";

export interface ResourcePageProps {
  resource: ResourceDef;
  /** Defaults to /api/admin/<slug> */
  endpoint?: string;
}

export function ResourcePage({ resource, endpoint }: ResourcePageProps) {
  ...
  return (
    <div data-testid={`cms-resource-${resource.slug}`}>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={reload}
            className="inline-flex items-center gap-1 px-3 py-2 text-sm rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-100"
            data-testid={`cms-refresh-${resource.slug}`}
          >
          ...
          {!resource.readOnly && (
            <button
              ...
              data-testid={`cms-new-${resource.slug}`}
              className="inline-flex items-center gap-1 px-3 py-2 text-sm rounded-lg bg-blue-600 hover:bg-blue-700 text-white"
            >
